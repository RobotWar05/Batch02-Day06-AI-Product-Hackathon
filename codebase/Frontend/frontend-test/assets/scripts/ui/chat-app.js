...
const scenarios = {
  happy: "Tìm 2 vé tàu hỏa từ Sài Gòn đi Đà Nẵng",
  low: "Tìm vé đi Đà Nẵng",
...
function addMessage(type, content, meta = {}) {
  state.messages.push({ id: createMessageId(), type, content, meta });
  render();
}
...
function renderUserBubble(text) {
  return `
    <div class="flex justify-end">
      <div class="max-w-[80%] rounded-xl rounded-tr-md bg-primary px-4 py-3 text-sm text-white shadow-ambient">
        ${escapeHtml(text)}
      </div>
    </div>
  `;
...
function renderBotBubble(text) {
  return `
    <div class="flex items-start gap-3">
      ...
      <div class="max-w-[85%] rounded-xl rounded-tl-md border border-outline bg-surface px-4 py-3 text-sm text-text shadow-ambient">
        ${escapeHtml(text)}
      </div>
    </div>
  `;
...
function renderTypingBubble() {
  return `
    <div class="flex items-start gap-3">
      ...
      <div class="rounded-xl rounded-tl-md border border-outline bg-surface px-4 py-3 text-sm text-muted shadow-ambient">
...
function renderClarification(slots) {
  ...
  const needDestination = slots.missing_fields.includes("destination");
  ...
  return `
    <div class="ml-0 md:ml-11 max-w-[620px] rounded-lg border-2 border-primary/20 bg-surface p-5 shadow-ambient">
      ...
      ${needDestination ? `
      <div class="mb-4">
        <p class="mb-2 text-sm font-semibold text-text">Bạn muốn đi đến đâu?</p>
        <div class="flex flex-wrap gap-2">
          ${quickButton("destination", "Hà Nội")}
          ${quickButton("destination", "TP.HCM")}
          ${quickButton("destination", "Đà Nẵng")}
          ${quickButton("destination", "Nha Trang")}
        </div>
      </div>` : ""}
      ${needTransport ? `
      <div class="mb-1">
...
function renderTripWidget(slots) {
  ...
  return `
    <div class="ml-0 md:ml-11 max-w-[620px] overflow-hidden rounded-lg border border-outline bg-surface shadow-ambient">
...
function renderResults(slots) {
  const flights = getSearchResults("flight", slots);
  const trains = getSearchResults("train", slots);
  ...
  const cheapestFlight = [...flights].sort((a, b) => a.total_price_vnd - b.total_price_vnd)[0];
  const preferredBest = slots.preferred_provider ? flights.find((item) => item.provider === slots.preferred_provider) : null;
    ...
    ? `<div class="mb-3 rounded-lg border border-primary/20 bg-secondary-container px-4 py-3 text-sm text-text">
    ...
    : "";
  return `
    <div class="ml-0 md:ml-11 max-w-[760px]">
      <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 class="text-lg font-bold text-text">Kết quả từ Data</h3>
          <p class="text-sm text-muted">${slots.origin} -> ${slots.destination} · giá mock không đổi theo ngày · ${tripCatalogSummary.total} chuyến</p>
        </div>
        <span class="w-fit rounded-full bg-secondary-container px-3 py-1 text-xs font-bold text-secondary">mock không cố định ngày</span>
      </div>
      ${cheaperHint}
      ${renderResultSection("flight", "Máy bay", "Lọc theo tuyến trong bộ Data, ưu tiên giá thấp và hãng bạn nêu nếu có.", visibleFlights, flights.length)}
      ${renderResultSection("train", "Tàu hỏa", "Lọc theo tuyến trong bộ Data để so sánh với máy bay.", visibleTrains, trains.length)}
    </div>
  `;
}
...
function renderResultSection(mode, title, subtitle, items, total) {
  ...
  return `
    <section class="mb-4 rounded-lg border border-outline bg-surface-low p-3">
      <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
...
function renderResultCard(item) {
  ...
  return `
    <article class="rounded-lg border border-outline bg-surface p-4 shadow-ambient transition hover:-translate-y-0.5 hover:shadow-floating">
...
function addOrReplaceWidget() {
  ...
  addMessage("bot", "Thông tin đã được cập nhật. Dưới đây là các chuyến đi phù hợp:");
